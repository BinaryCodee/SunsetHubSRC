...
public class TpAllCMD implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length != 1) {
            player.sendMessage(ChatUtils.getConfigText("modules.teleports.messages.usage-tpall"));
            return true;
        }
        ...
        player.sendMessage(ChatUtils.getConfigText("modules.teleports.messages.tpall-success").replace("{player}", target.getName()));
        return true;
    }
}
